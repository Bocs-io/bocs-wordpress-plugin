...
class Bocs_Checkout {
    ...
    public function __construct() {
        ...
        // Add action to check and disable the save payment method checkbox
        add_action('wp_enqueue_scripts', array($this, 'check_and_disable_save_payment_checkbox'));
    ...
    public function add_checkout_login_styles() {
        ...
        // Auto-check the "Save payment information" checkbox for BOCS subscriptions
        if ($this->cart_contains_bocs_subscription()) {
            // JavaScript for standard checkout
            wp_add_inline_script('woocommerce', '
                jQuery(document).ready(function($) {
                    // Check and disable the save payment method checkbox
                    $(document.body).on("updated_checkout payment_method_selected", function() {
                        // Find save payment checkbox - handle multiple possible formats for different Stripe versions
                        var $savePaymentCheckbox = $(
                            // Legacy Stripe
                        ...
                        if ($savePaymentCheckbox.length) {
                            ...
                            $savePaymentCheckbox.each(function() {
                                var $label = $(this).closest("label, .wc-stripe-save-payment-method");
                    ...
                    // Trigger immediately in case the checkout is already loaded
                    $(document.body).trigger("updated_checkout");
                });
            ');
            
            // JavaScript for block-based checkout
            wp_add_inline_script('wc-blocks-checkout', '
                (function() {
                    // Function to handle blocks checkout save payment checkbox
                    function handleBlocksCheckout() {
                        // Wait for elements to be present in DOM
                        const checkForSaveElements = setInterval(function() {
                        ...
                        setTimeout(function() {
                            clearInterval(checkForSaveElements);
                        }, 30000);
                    }
                    ...
                    if (document.readyState === "complete" || document.readyState === "interactive") {
                        ...
                        document.addEventListener("DOMContentLoaded", handleBlocksCheckout);
                    }
                    
                    // Also handle potential re-renders
                    document.addEventListener("blocks_checkout_update", handleBlocksCheckout);
                })();
            ');
        }
    }
     ...
     * Check and disable the save payment method checkbox
     */
    public function check_and_disable_save_payment_checkbox() {
        // Only proceed if on checkout page and cart contains BOCS subscription
        if (!is_checkout() || !$this->cart_contains_bocs_subscription()) {
            return;
        }
        ...
        wp_enqueue_script(
            'bocs-custom-stripe-checkout',
            plugin_dir_url(dirname(__FILE__)) . 'assets/js/custom-stripe-checkout.js',
            array('jquery'),
            filemtime(plugin_dir_path(dirname(__FILE__)) . 'assets/js/custom-stripe-checkout.js'),
            true
        );
