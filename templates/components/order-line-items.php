...
// Add inline styles for the discount text
?>
<style>
.bocs-order-table .discount-text {
    display: block;
    font-size: 0.85em;
    color: #888;
    margin-top: 3px;
    ...
    <table class="bocs-order-table"<?php echo $has_empty_products ? ' style="display:none;"' : ''; ?>>
        <thead>
            <tr>
                ...
                <th style="text-align: right;"><?php esc_html_e('Total', 'bocs-wordpress'); ?></th>
                <th style="text-align: right;"><?php esc_html_e('GST', 'bocs-wordpress'); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php
                ...
                <td data-title="<?php esc_attr_e('Total', 'bocs-wordpress'); ?>" class="text-right">
                    ...
                    if (function_exists('wc_price')) {
                        ...
                        if (!empty($discount_percent) && ($discount_type === 'percent' || $discount_type === 'percentage')) {
                            $item_discount = floatval($discount_percent) / 100 * $display_line_total;
                            echo '<br><span class="discount-text">' . wp_kses_post(wc_price($item_discount) . ' discount') . '</span>';
                        }
                    } else {
                        ...
                        if (!empty($discount_percent) && ($discount_type === 'percent' || $discount_type === 'percentage')) {
                            $item_discount = floatval($discount_percent) / 100 * $display_line_total;
                            echo '<br><span class="discount-text">$' . number_format($item_discount, 2) . ' discount</span>';
                        }
                    }
                    ?>
                </td>
        ...
        <tfoot>
        <?php
            // Calculate the total GST for display (including shipping tax)
            $total_tax = $calculated_tax + $shipping_tax;

            // Define display_tax for use in total calculation
            $display_tax = ($total_tax > 0) ? $total_tax : (is_array($tax) ? 0 : (float)$tax);
            ?>
            <tr class="bocs-subtotal-row">
                <th colspan="4"><?php esc_html_e('Subtotal', 'bocs-wordpress'); ?></th>
            ...
            <tr class="bocs-discount-row">
                <th colspan="4">
                    ...
                    $discount_label = esc_html__('Discount Total', 'bocs-wordpress');
            ...
            <tr class="bocs-shipping-row">
                <th colspan="4"><?php esc_html_e('Shipping', 'bocs-wordpress'); ?></th>
            ...
            <tr class="bocs-total-row">
                ...
                <td class="text-right order-total" data-title="<?php esc_attr_e('Tax', 'bocs-wordpress'); ?>">
                    ...
                    // Use the calculated total (tax excluded) or fall back to the provided total
                    $display_total = ($calculated_total > 0) ? $calculated_total : $total;

                    if (function_exists('wc_price')) {
                        echo wp_kses_post(wc_price($total_tax));
                    } else {
                        echo esc_html('$' . number_format($total_tax, 2));
                    }
            ...
            <tr class="bocs-final-total-row">
                <th colspan="4"><?php esc_html_e('Total (inc tax)', 'bocs-wordpress'); ?></th>
                <td class="text-right" data-title="<?php esc_attr_e('Total (inc tax)', 'bocs-wordpress'); ?>">
                    <?php
                    if (function_exists('wc_price')) {
