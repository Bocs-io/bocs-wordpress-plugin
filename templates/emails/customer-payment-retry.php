...
<div class="bocs-email-container">
    <div class="bocs-email-content">
        ...
        <h2 class="bocs-header"><?php esc_html_e('Payment Retry Notification', 'bocs-wordpress'); ?></h2>

        <p><?php printf(esc_html__('Hi %s,', 'bocs-wordpress'), esc_html($order->get_billing_first_name())); ?></p>

        <?php if (!empty($retry_date)) : ?>
            <p><?php printf(
                esc_html__('We wanted to let you know that we will retry charging your payment method for your subscription renewal on %s.', 'bocs-wordpress'),
                '<strong>' . esc_html($retry_date) . '</strong>'
            ); ?></p>
        <?php else : ?>
            <p><?php esc_html_e('We wanted to let you know that we will retry charging your payment method for your subscription renewal soon.', 'bocs-wordpress'); ?></p>
        <?php endif; ?>
        ...
        <p><?php esc_html_e('You don\'t need to do anything, but if you\'d like to update your payment information before the retry, you can do so from your account.', 'bocs-wordpress'); ?></p>

        <a href="<?php echo esc_url(wc_get_account_endpoint_url('payment-methods')); ?>" class="bocs-button">
            <?php esc_html_e('Update Payment Method', 'bocs-wordpress'); ?>
        </a>

        <h3 class="bocs-header"><?php esc_html_e('Order Details', 'bocs-wordpress'); ?></h3>
    </div>
</div>
