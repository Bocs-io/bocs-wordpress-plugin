...
$content .= '<p style="margin: 0 0 16px;">' . esc_html__('Your subscription has been cancelled as requested. Below are the details of your cancelled subscription for your reference:', 'bocs-wordpress') . '</p>';

// Cancellation notification box
$content .= '<div style="background-color: #ffebee; border-left: 4px solid #f44336; padding: 15px 20px; margin-bottom: 30px; border-radius: 4px;">';
$content .= '<p style="margin: 0 0 16px; color: #d32f2f; font-weight: 600;">' . esc_html__('Subscription Cancelled', 'bocs-wordpress') . '</p>';
$content .= '<p style="margin: 0 0 16px;">' . esc_html__('Your subscription has been cancelled and you will no longer be billed for this service.', 'bocs-wordpress') . '</p>';
...
if (!empty($cancellation_reason)) {
    $content .= '<p style="margin: 0 0 16px;"><strong>' . esc_html__('Reason for cancellation:', 'bocs-wordpress') . '</strong> ' . esc_html($cancellation_reason) . '</p>';
}
...
if (isset($subscription['updatedAt']) || isset($subscription['updatedAtGmt'])) {
    ...
    $content .= '<p style="margin: 0 0 16px;"><strong>' . esc_html__('Cancelled on:', 'bocs-wordpress') . '</strong> ' . esc_html($cancel_date->format('F j, Y')) . '</p>';
}
...
$content .= '<h3 style="color: #3C7B7C; margin-top: 0;">Want to Resubscribe?</h3>';
$content .= '<p style="margin-bottom: 20px;">If you change your mind, you can always sign up for a new subscription from our site.</p>';

// Shop URL - adjust as needed
$shop_url = get_permalink(wc_get_page_id('shop'));
if ($shop_url) {
    $content .= '<a href="' . esc_url($shop_url) . '" style="display: inline-block; background-color: #3C7B7C; color: #ffffff; font-size: 16px; font-weight: bold; line-height: 100%; text-decoration: none; padding: 12px 25px; border-radius: 4px;">';
    $content .= esc_html__('Shop Now', 'bocs-wordpress');
    $content .= '</a>';
}
$content .= '</div>';
...
// Thank you message
$content .= '<p style="margin: 0 0 16px;">' . esc_html__('Thank you for being our customer. We hope to see you again soon!', 'bocs-wordpress') . '</p>';
