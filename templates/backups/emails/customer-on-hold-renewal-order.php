...
// On-hold message
$content .= '<p style="margin: 0 0 16px;">' . esc_html__('Thanks for your renewal order. It\'s on hold until we confirm payment has been received. In the meantime, here\'s a reminder of your order details:', 'bocs-wordpress') . '</p>';
...
$content .= '<p style="margin: 0 0 16px; color: #ffa000; font-weight: 600;">' . esc_html__('Order On Hold', 'bocs-wordpress') . '</p>';
$content .= '<p style="margin: 0 0 16px;">' . esc_html__('Your order is on hold until we confirm payment has been received. You\'ll receive another email once your payment has been confirmed.', 'bocs-wordpress') . '</p>';
$content .= '</div>';

// Payment button for bank transfers
if ($order->get_payment_method() == 'bacs') {
    $content .= '<div style="margin-bottom: 30px; text-align: center;">';
    $content .= '<h3 style="margin-bottom: 10px;">' . esc_html__('Our Bank Details', 'bocs-wordpress') . '</h3>';
    
    ob_start();
    do_action('woocommerce_email_before_order_table', $order, $sent_to_admin, $plain_text, $email);
    $bank_details = ob_get_clean();
    
    // Replace any WooCommerce purple with Bocs teal
    if (function_exists('bocs_replace_woocommerce_colors')) {
        $bank_details = bocs_replace_woocommerce_colors($bank_details);
    } else {
        $bank_details = str_replace("#7f54b3", "#3C7B7C", $bank_details);
    }
...
$content .= '<p style="margin: 0 0 16px;">' . esc_html__('If you have any questions about your order, please contact our customer support team.', 'bocs-wordpress') . '</p>';
