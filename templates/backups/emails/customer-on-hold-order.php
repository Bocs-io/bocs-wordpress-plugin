...
// On-hold message
$content .= '<p style="margin: 0 0 16px;">Thanks for your order. It\'s on hold until we confirm your payment. We will let you know once your payment has been confirmed.</p>';

// On-hold status box
$content .= '<div style="background-color: #f3e5f5; border-left: 4px solid #7b1fa2; padding: 15px 20px; margin-bottom: 30px; border-radius: 4px;">';
$content .= '<p style="margin: 0 0 16px; color: #7b1fa2; font-weight: 600;">Payment Required</p>';
$content .= '<p style="margin: 0 0 16px;">Your order is currently on hold. Please complete your payment to proceed with processing.</p>';

// Add payment button if needed
if ($order->needs_payment()) {
    $pay_url = $order->get_checkout_payment_url();
    if (!empty($pay_url)) {
        $content .= '<div style="text-align: center; margin: 20px 0 10px;">';
