...
// Refund message based on full or partial refund
if ($partial_refund) {
    $content .= '<p style="margin: 0 0 16px;">Your order on ' . esc_html(get_bloginfo('name', 'display')) . ' has been partially refunded. There are more details below for your reference:</p>';

    // Partial refund notification box
    ...
    $content .= '<p style="margin: 0 0 16px; color: #ffa000; font-weight: 600;">Partial Refund Processed</p>';
    $content .= '<p style="margin: 0 0 16px;">A partial refund of ' . wp_kses_post(wc_price($refund_amount, array('currency' => $order->get_currency()))) . ' has been issued on your order.</p>';
    $content .= '</div>';
} else {
    $content .= '<p style="margin: 0 0 16px;">Your order on ' . esc_html(get_bloginfo('name', 'display')) . ' has been refunded in full. There are more details below for your reference:</p>';

    // Full refund notification box
    ...
    $content .= '<p style="margin: 0 0 16px; color: #4caf50; font-weight: 600;">Full Refund Processed</p>';
    $content .= '<p style="margin: 0 0 16px;">A full refund of ' . wp_kses_post(wc_price($order->get_total(), array('currency' => $order->get_currency()))) . ' has been issued on your order.</p>';
    $content .= '</div>';
}
...
$content .= '<p style="margin: 0 0 16px;">If you have any questions about this refund, please contact our customer support team for assistance.</p>';
$content .= '</div>';
...
$content .= '<p style="margin: 0 0 16px;">We appreciate your business and thank you for shopping with Bocs.</p>';
$content .= '</div>';
$content .= '</div>';
