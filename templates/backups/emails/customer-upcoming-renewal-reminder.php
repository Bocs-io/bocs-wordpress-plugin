...
// Renewal message
$content .= '<p style="margin: 0 0 16px;">' . esc_html__('This is a friendly reminder that your subscription renewal payment will be automatically processed soon.', 'bocs-wordpress') . '</p>';
...
$content .= '<p style="margin: 0 0 16px; color: #ffa000; font-weight: 600;">' . esc_html__('Upcoming Renewal', 'bocs-wordpress') . '</p>';
...
if ($subscription->get_date('next_payment')) {
    ...
    $content .= sprintf(
        ...
        '<strong>' . wp_kses_post(wc_price($subscription->get_total(), array('currency' => $subscription->get_currency()))) . '</strong>',
...
if (!empty($payment_method_title)) {
    $content .= '<div style="background-color: #f8f9fa; padding: 15px 20px; margin-bottom: 25px; border-radius: 4px; border: 1px solid #e5e5e5;">';
    $content .= '<h3 style="color: #333333; font-size: 16px; font-weight: 600; margin-bottom: 15px;">' . esc_html__('Payment Information', 'bocs-wordpress') . '</h3>';
    $content .= '<p style="margin: 0 0 10px;"><strong>' . esc_html__('Payment Method:', 'bocs-wordpress') . '</strong> ' . esc_html($payment_method_title) . '</p>';
    
    if ($subscription->get_payment_method() === 'stripe') {
        $content .= '<p style="margin: 0 0 16px;">' . esc_html__('Your card on file will be automatically charged.', 'bocs-wordpress') . '</p>';
    }
    $content .= '</div>';
}
...
$content .= '<p style="margin: 0 0 16px;">' . esc_html__('If you need to update your payment method or have any questions about your subscription, please contact our customer support team.', 'bocs-wordpress') . '</p>';
