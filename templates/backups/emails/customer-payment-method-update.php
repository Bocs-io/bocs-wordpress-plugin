...
// Payment method update message
$content .= '<p style="margin: 0 0 16px;">' . esc_html__('The payment method for your subscription has been updated.', 'bocs-wordpress') . '</p>';
...
$content .= '<p style="margin: 0 0 16px;">' . esc_html__('Your payment method has been successfully updated and will be used for future subscription renewals.', 'bocs-wordpress') . '</p>';
$content .= '</div>';

// Check for Bocs App attribution
$source_type = get_post_meta($subscription->get_parent_id(), '_wc_order_attribution_source_type', true);
$utm_source = get_post_meta($subscription->get_parent_id(), '_wc_order_attribution_utm_source', true);
$bocs_id = get_post_meta($subscription->get_parent_id(), '__bocs_bocs_id', true);
...
if (!empty($payment_method)) {
    $content .= '<div style="background-color: #f8f9fa; padding: 20px; margin-bottom: 30px; border-radius: 4px; border: 1px solid #e5e5e5;">';
    $content .= '<h3 style="color: #333333; font-size: 16px; font-weight: 600; margin-bottom: 15px;">' . esc_html__('New Payment Method', 'bocs-wordpress') . '</h3>';
    $content .= '<p style="margin: 0 0 16px;"><strong>' . esc_html($payment_method) . '</strong></p>';
    $content .= '</div>';
}

// Order details heading
$content .= '<h2 style="display: block; color: #333333; font-family: \'Helvetica Neue\', Helvetica, Arial, sans-serif; font-size: 22px; font-weight: 500; line-height: 130%; margin: 30px 0 18px; text-align: left;">' . esc_html__('Subscription Details', 'bocs-wordpress') . '</h2>';
$content .= '</div>';
...
$content .= sprintf(__('[Order #%s] (%s)', 'bocs-wordpress'), $subscription->get_order_number(), date_i18n(wc_date_format(), strtotime($subscription->get_date_created())));
$content .= '</h2>';
...
$content .= '<p style="margin: 0 0 16px;">' . esc_html__('If you have any questions about your payment method update, please contact our customer support team.', 'bocs-wordpress') . '</p>';
