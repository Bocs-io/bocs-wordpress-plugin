...
// Paused message
$content .= '<p style="margin: 0 0 16px;">' . esc_html__('Your subscription has been paused. Your subscription details are shown below for your reference:', 'bocs-wordpress') . '</p>';

// Pause notification box
$content .= '<div style="background-color: #eff8fb; border-left: 4px solid #03a9f4; padding: 15px 20px; margin-bottom: 30px; border-radius: 4px;">';
$content .= '<p style="margin: 0 0 16px; color: #03a9f4; font-weight: 600;">' . esc_html__('Subscription Paused', 'bocs-wordpress') . '</p>';

// Pause details
if ($subscription->get_date('next_payment')) {
    $content .= '<p style="margin: 0 0 16px;">';
    $content .= sprintf(
        esc_html__('Your subscription is paused. It will automatically resume on %s.', 'bocs-wordpress'),
        '<strong>' . esc_html(date_i18n(wc_date_format(), strtotime($subscription->get_date('next_payment')))) . '</strong>'
    );
    $content .= '</p>';
} else {
    $content .= '<p style="margin: 0 0 16px;">' . esc_html__('Your subscription is paused. You can resume it at any time from your account.', 'bocs-wordpress') . '</p>';
}
$content .= '</div>';
...
$content .= '<p style="margin: 0 0 16px;">' . esc_html__('If you\'d like to resume your subscription sooner, you can do so from your account.', 'bocs-wordpress') . '</p>';
